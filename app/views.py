...
# securely in Django settings or environment variables, NOT directly in views.
RAZORPAY_KEY_ID = "rzp_test_P8Jqpa3xzoFUQ0"
RAZORPAY_KEY_SECRET = "oNsarG9VffDuiVnO1bLZesXN" # Replace with your actual secret key from Razorpay Dashboard
...
def member_required(view_func):
    """
    Decorator for views that checks if the user is logged in as a member.
...
def userpage(request):
    ...
    context = {
        ...
        'now': timezone.now(),
        # RAZORPAY_KEY_ID is now passed only to the specific payment page
    }
...
# NEW: View for Librarian Book Management (List all books)
@librarian_required
...
def librarian_dashboard(request):
    ...
    # Filter for rejected or failed payments
    rejected_transactions = Borrowing.objects.filter(status__in=['rejected_purchase', 'payment_failed']).select_related('member', 'book').order_by('-approval_date')
